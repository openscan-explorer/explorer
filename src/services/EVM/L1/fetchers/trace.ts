...
export class TraceFetcher {
   ...
   */
  async traceTransaction(txHash: string, config?: TraceCallConfig): Promise<TraceResult> {
    try {
      const result = await this.rpcClient.call<TraceResult>("debug_traceTransaction", [
        txHash,
        config || {},
      ]);
      return result;
