 ...
 */
export function transformBNBBlockToBlock(rpcBlock: BNBBlock): Block {
  ...
  return {
    ...
    transactions: Array.isArray(rpcBlock.transactions)
      ? rpcBlock.transactions.map((tx) =>
          typeof tx === "string" ? tx : (tx as BNBTransaction).hash,
        )
      : [],
    size: rpcBlock.size,
 ...
 */
export function transformBNBTransactionToTransaction(
  rpcTx: BNBTransaction,
  receipt?: BNBTransactionReceipt | null,
): Transaction {
