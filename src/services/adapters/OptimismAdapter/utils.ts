 ...
 */
export function transformOptimismBlockToBlock(rpcBlock: OptimismBlock): Block {
  ...
  return {
    ...
    transactions: Array.isArray(rpcBlock.transactions)
      ? rpcBlock.transactions.map((tx) =>
          typeof tx === "string" ? tx : (tx as OptimismTransaction).hash,
        )
      : [],
    size: rpcBlock.size,
 ...
 */
export function transformOptimismTransactionToTransaction(
  rpcTx: OptimismTransaction,
  receipt?: OptimismTransactionReceipt | null,
): Transaction {
