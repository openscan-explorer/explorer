...
const ContractsSection: React.FC = () => {
	...
	const [stdSources, setStdSources] = useState(JSON.stringify({
		"MyContract.sol": {
			content: "// SPDX-License-Identifier: MIT\npragma solidity ^0.8.0;\n\ncontract MyContract {\n    uint256 public value;\n    \n    constructor(uint256 _value) {\n        value = _value;\n    }\n}"
		}
	}, null, 2));
	const [stdSettings, setStdSettings] = useState(JSON.stringify({
		optimizer: {
			enabled: true,
			runs: 200
		},
	...
	const verifyMetadata = async () => {
		...
		try {
			...
			const response = await fetch(`${SOURCIFY_API}/v2/verify/metadata/${metaChainId}/${metaAddress}`, {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
	...
	const importFromEtherscan = async () => {
		...
		try {
			...
			const url = new URL(`${SOURCIFY_API}/v2/verify/etherscan/${etherscanChainId}/${etherscanAddress}`);
			if (etherscanApiKey) {
	...
	const verifySimilarity = async () => {
		...
		try {
			...
			const response = await fetch(`${SOURCIFY_API}/v2/verify/similarity/${simChainId}/${simAddress}`, {
				method: "POST",
			});
