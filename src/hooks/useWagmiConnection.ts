...
export const useWagmiConnection = () => {
  const { isConnected, isConnecting, isReconnecting, isDisconnected, address, connector, status } =
    useAccount();
  ...
  useEffect(() => {
    ...
    const fullyConnected =
      isConnected && !!address && !isConnecting && !isReconnecting && connections.length > 0;
