 ...
 */
export function convertEthUnits(value: string, from: EthUnit): Record<EthUnit, string> | null {
 ...
 */
export function computeKeccak(input: string, inputType: SolidityType): KeccakResults {
  if (!input.trim()) {
    return {};
  }
  ...
  try {
    ...
    const solidityEncodePacked = solidityPackedKeccak256([inputType], [parsedValue]);
    ...
    const isFunctionSignature = inputType === "string" && functionSigPattern.test(input.trim());
